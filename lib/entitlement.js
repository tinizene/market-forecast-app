...
async function deriveFromStripe(customerId) {
  ...
  for (const s of (sessions && sessions.data) || []) {
    if (s.metadata && s.metadata.product === 'course' && s.payment_status === 'paid') {
      // Earliest paid course session wins: the included window runs from the first
      // purchase, so buying twice cannot quietly extend it.
      if (courseAt == null || s.created < courseAt) courseAt = s.created;
    }
  }
...
async function checkEntitlement(req, res) {
  ...
  if (ent && ent.exp && ent.exp > now) {
    ...
    // issued (refund, chargeback, cancellation). This check is what makes revocation
    // take effect immediately rather than at the next refresh.
    ...
    // A cookie issued before this change carries only {sub, exp}: a subscriber with
    // no course. Honour it as ideas-until-exp so existing subscribers are not logged
    // out by the upgrade; it re-derives properly at the next refresh.
  ...
  if (cus && cus.cus) {
    try {
      ...
      // On a Stripe outage, fail closed (no access) rather than open.
