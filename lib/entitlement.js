...
async function stripeRequest(method, path, formObj) {
  ...
  // STRIPE_API_BASE exists so the auth/entitlement tests can point Stripe at a local
  // stub. Never set in production; unset, this is exactly the old hardcoded host.
  let url = `${process.env.STRIPE_API_BASE || 'https://api.stripe.com'}${path}`;
...
// ---- single-use claims and rate limiting (both need the store) ----
//
// Used by api/auth.js for magic links. Both return null when no store is configured,
// which the caller must treat as "cannot do this safely" rather than as permission —
// a single-use token that cannot be burned is replayable, and an unlimited link
// endpoint is an email-bombing tool aimed at a customer's inbox.

// Claims a key exactly once. Returns true the first time and false every time after,
// so a replayed magic link is refused by the second caller rather than by trust.
async function claimOnce(key, ttlSec) {
  if (!storeConfigured()) return null;
  const result = await kvCommand(['SET', key, '1', 'NX', 'EX', String(ttlSec)]);
  // Upstash returns "OK" when NX succeeded and null when the key already existed.
  // Return a strict boolean: null is reserved for "no store", and a caller must not
  // have to tell those two apart by accident.
  return result === 'OK';
}

// Fixed-window counter. Coarse by design: the point is to stop bulk abuse, not to
// meter precisely, and a sliding window would cost more round trips than it is worth.
async function hitRateLimit(key, limit, windowSec) {
  if (!storeConfigured()) return null;
  const count = await kvCommand(['INCR', key]);
  const n = typeof count === 'number' ? count : parseInt(count, 10) || 1;
  if (n === 1) await kvCommand(['EXPIRE', key, String(windowSec)]);
