...
const { group, fixed, plural, pct, escape, htmlTable, page } = require('./render-helpers.js');
...
function markdown(data) {
  ...
  push(`p (upper tail)     ${fixed(u.pUpper, 4)}`);
...
module.exports = { markdown, DERIVATIONS, oneIn };
...
hold         1 &minus; ${fixed(row.rtp, 6)} = ${fixed(1 - row.rtp, 6)}</pre>
  ...
  return page({
    title: 'Africa Numbers Game Mathematics',
    stamp: ['Game mathematics', 'Africa Numbers', 'Computed, not transcribed'],
    heading: 'Every outcome, counted',
    standfirst: `The board has ${group(OUTCOMES)} outcomes, so nothing here is sampled. Win counts come from
      asking the settlement code about every selection the product accepts, against every
      outcome it can produce &mdash; a million questions per bet type.`,
    body: `
