...
test('the published sheet is the one this tree produces', () => {
  ...
  // Everything up to Rounding is enumeration: exact, cheap, and comparable
  // here. The sampled section that follows is regenerated and compared by
  // npm run mathsheet:check, which is a separate step because it takes seconds.
  const board = markdown(math.compute({ samples: 1 })).split('## Rounding')[0];
  ...
  assert.match(onDisk, /chi-square\s+[\d.]+ on 999 degrees of freedom/,
    'and the document still records the sampled figure');
});
