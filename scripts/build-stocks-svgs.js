...
//
// Palette is the dark-theme set from course/CLAUDE.md. Every colour used for text is
// checked against the #0f172a background at build time and must clear WCAG 2.2 AA at
// 4.5:1 — the script exits nonzero if one does not, so a future palette edit cannot
// quietly ship unreadable labels.
//
...
function chain(items, o = {}) {
  ...
  items.forEach((it, i) => {
    ...
    if (i < n - 1) {
      ...
      out.push(txt(mx, y + boxH / 2 + 5, 'then', { anchor: 'middle', size: 12, fill: C.muted }));
    }
  });
  return out;
}

// Two or three bordered columns of text.
...
// Well-formedness check: the app injects this markup directly, so a malformed
// diagram would break the lesson rather than just look wrong.
const { XMLParser, XMLValidator } = (() => {
  try { return require('fast-xml-parser'); } catch (e) { return {}; }
})();

let bad = 0;
for (const [key, markup] of Object.entries(SVGS)) {
  if (XMLValidator) {
    const res = XMLValidator.validate(markup);
    if (res !== true) { console.error(`  ! ${key}: ${res.err.msg} (line ${res.err.line})`); bad++; }
  } else {
    // Minimal fallback when the parser is unavailable: unescaped bare ampersands and
    // stray angle brackets in text nodes are the failure modes that have bitten before.
    const textNodes = [...markup.matchAll(/>([^<>]*)</g)].map((m) => m[1]);
    const offending = textNodes.filter((t) => /&(?!amp;|lt;|gt;|quot;|apos;|#)/.test(t));
    if (offending.length) { console.error(`  ! ${key}: unescaped ampersand in text`); bad++; }
    const tags = (markup.match(/<[a-z]/g) || []).length;
    const closes = (markup.match(/<\/[a-z]+>|\/>/g) || []).length;
    if (tags !== closes) { console.error(`  ! ${key}: ${tags} open tags vs ${closes} closes`); bad++; }
  }
}
if (bad) { console.error(`${bad} diagram(s) failed validation`); process.exit(1); }
