...
- Section 2 (Performance Review) is mandatory every run: a markdown table reviewing every still-open idea from the prior context above (re-verify each pair's current level via search), a "Running Hit Rate: X played out / Y invalidated / Z still pending" line, and note which idea needs closest watching. The table must include a **"Distance from Target"** column (see the next rule) — the published track record is parsed from these columns, so a figure left in prose never reaches the reader.
- Section 2 DISTANCE FROM TARGET (mandatory for every idea that RESOLVES this run, win or loss): the performance table must carry a **"Distance from Target"** column, and every resolving idea must state two figures in it, computed from prices already published for that idea. Use the entry-zone midpoint and the target-zone midpoint where a zone was given:
  - **Gap to target** = |resolution price - target midpoint| / target midpoint x 100, to two decimals. Phrase it as how far the pair finished FROM the target zone (e.g. "finished 4.71% away from target").
  - **Progress to target** = (resolution price - entry midpoint) / (target midpoint - entry midpoint) x 100, to the nearest whole percent, SIGNED. This formula is direction-agnostic — it reads correctly for both longs and shorts, because numerator and denominator flip together — so do NOT special-case shorts. 100% means the target was reached; 0% means the pair never moved toward it; a NEGATIVE figure means it moved against the thesis by that share of the intended move (e.g. "-49% — it travelled roughly half the intended distance in the wrong direction").
  State both, in that order, with the arithmetic visible (entry midpoint, target midpoint, resolution price). For a played-out idea the gap will be at or near 0% and progress at or above 100% — still show both, because "we hit target with 8% to spare" and "we scraped it by 0.1%" are different results and a reader deserves to tell them apart. If an idea was published with no explicit target, write "no published target" rather than inventing one.
  This figure is not decoration: an invalidated idea whose progress reads -49% was wrong about DIRECTION, while one reading +80% was right about direction and wrong about timing or stop placement, and those are different lessons. Say which of the two it was in the Educational Breakdown's "Match or divergence" part.
